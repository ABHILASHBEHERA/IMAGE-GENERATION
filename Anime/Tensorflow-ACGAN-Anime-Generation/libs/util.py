...
def preprocessing(preproc_dir, img_dir, tag_path, eye_map, hair_map):
    ...
    with open(tag_path, 'r') as f:
        for idx, row in enumerate(csv.reader(f)):
            ...
            feat = cv2.imread(img_path)
            feat = cv2.resize(feat, (img_size, img_size))
            ...
            feat_p5 = imutils.rotate(feat, 5)
            feat_p5 = cv2.resize(feat_p5, (resieze , resieze))
            feat_p5 = crop_center(feat_p5, img_size,img_size)

            attrib_tags.append([hair_idx, eyes_idx])
            img_feat.append(feat_p5)

            feat_m5 = imutils.rotate(feat, -5)
            feat_m5 = cv2.resize(feat_m5, (resieze, resieze))
...
def dump_img(img_dir, img_feats, iters, img_size = 96):
    ...
    for idx, img_feat in enumerate(img_feats):
        ...
        # misc.imsave(path, img_feat)
        data = im.fromarray(img_feat,'RGB')
        data.show()
        data.save('/content/drive/MyDrive/Computational Creativity/Anime/test_'+str(iters)+'_'+str(idx)+'_pic.png')
